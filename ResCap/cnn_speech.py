...
from fbanks_load import load_mnist,read
...
class Net(nn.Module):
    def __init__(self):
        ...
        self.fc1 = nn.Linear(128*6*2, 328)
    ...
    def forward(self, x):
        ...
        # print('x2 size',x.shape)
        x = F.relu(F.max_pool2d(self.conv3(x), 2))
        # print('x3 size',x.shape)
        x = x.view(-1, 128*6*2)
        x = F.relu(self.fc1(x))
        x = F.relu(self.fc2(x))
...
def main():
    ...
    parser.add_argument('--epochs', type=int, default=40, metavar='N',
                        help='number of epochs to train (default: 26)')
