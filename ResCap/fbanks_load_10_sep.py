...
def read(label):
    # test_data = np.load('home/xps15/Desktop/MultiMnist_V2/data/multimnist/multimnist_test_5_py3.npy')
    # train_data = np.load('home/xps15/Desktop/MultiMnist_V2/data/multimnist/multimnist_train_1_py3.npy')
    # if label ==1:
    ...
    if label ==1:
        test_data = np.load('/home/xps15/Desktop/capsulenet/data/word_mix/new/fb_features_single_sep_test_spkind_80.npy',encoding = 'bytes')
        train_data = np.load('/home/xps15/Desktop/capsulenet/data/word_mix/new/fb_features_single_sep_train_spkind_80.npy',encoding = 'bytes')
        print('using speaker independent')
    if label ==2:
        test_data = np.load('/home/xps15/Desktop/capsulenet/data/word_mix/new/fb_features_ten_test_spkdep.npy',encoding = 'bytes')
    ...
    for i in range(train_size):
        ...
        train_tuple = (train_data[i][0].astype('float')/(train_data[i][0].max()-train_data[i][0].min()),train_data[i][1].astype('float')/(train_data[i][1].max()-train_data[i][1].min()),train_data[i][2]-1,train_data[i][3]-1)
        train_out.append(train_tuple)
    print('finish training load')
    test_out = []
    for i in range(test_size):
        # if test_data[i][0].shape[0] ==98:
        test_tuple = (test_data[i][0].astype('float')/(test_data[i][0].max()-test_data[i][0].min()),test_data[i][1].astype('float')/(test_data[i][1].max()-test_data[i][1].min()),test_data[i][2]-1,test_data[i][3]-1)
