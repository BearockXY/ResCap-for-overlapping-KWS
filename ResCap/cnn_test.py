...
class Net(nn.Module):
    def __init__(self):
        ...
        self.fc0 = nn.Linear(10*10*32*8,160)
        self.fc1 = nn.Linear(160, 512)
        self.fc2 = nn.Linear(512, 1024)
        self.fc3 = nn.Linear(1024,36*36)

    def forward(self, x):
        x = self.conv1(x)
        x = self.conv2(x)

        x = x.view(-1, 10*10*32*8)
        x = self.fc0(x)
        x = F.relu(self.fc1(x))
        x = F.relu(self.fc2(x))
        x = F.dropout(x, training=self.training)
        x = self.fc3(x)
        return x.view(-1,*[1,36,36])
...
def train(args, model, device, train_loader,test_loader, optimizer, epoch,ti):
    ...
    for batch_idx, (data, target1,target2) in enumerate(train_loader):
        ...
        data = data.view_as(torch.Tensor(args.batch_size,1,36,36)).float()
        data, target1,target2 = data.to(device), target1.to(device),target2.to(device)
        optimizer.zero_grad()

        output = model(data)

        loss = cap_loss(data,output)
...
def test(args, model,device,train_loss,test_loader,epoch,ti):
    ...
    with torch.no_grad():
        for data,target1,target2 in test_loader:
            ...
            output = model(data)
            test_loss += cap_loss(output, data) # sum up batch loss
