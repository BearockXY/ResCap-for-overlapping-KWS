...
from mfcc_load_10_single import load_mnist,read

def judge_correct(label1,pred1):
    label1 = label1.long()
    pred1 = pred1.long()
    correct = 0
    Miss = np.zeros([10,10])
...
class SpeechResModel(nn.Module):
    ...
    def forward(self, x,y_p=None):
        ...
        if y_p is None:  # during testing, no label given. create one-hot coding using `length`
            index1 = find_index(length)
            index = torch.zeros(length.size()).scatter_(1, index1.view(-1, 1).cpu().data, 1.)
...
def test(model, test_loader, args):
    ...
    for x, y  in test_loader:
        ...
        true_label = torch.zeros(y.size(0), 10).scatter_(1, y.view(-1, 1), 1.)
        ...
        result1, Miss = judge_correct(y,index_pred1.float())
        correct1 +=result1
        MissMat  +=Miss
