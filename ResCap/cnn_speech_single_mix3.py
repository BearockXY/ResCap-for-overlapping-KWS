...
def Find_correct(inputdata,target1,target2,target3):
    ...
    correct2= 0
    correct3 = 0
    index_pred1,index_pred2,index_pred3 = find_index(inputdata)
...
def train(args, model, device, train_loader,test_loader, optimizer, epoch,ti):
    ...
    model.train()
    train_loss = 0
    for batch_idx, (data, target1) in enumerate(train_loader):
...
def test(args, model,device,train_loss,test_loader,epoch,ti):
    ...
    with torch.no_grad():
        for data,target1 in test_loader:
            # target = tag(target1,target2)
            data = data.view_as(torch.Tensor(args.batch_size,1,98,60)).float()
            data, target1 = data.to(device), target1.to(device)

            output = model(data)
            test_loss += CELoss(output,target1)*data.size(0) # sum up batch loss
            # pred = output.max(1, keepdim=True)[1] # get the index of the max log-probability
            # correct += pred.eq(target.view_as(pred)).sum().item()
            result1,Miss = Find_correct(output,target1)
            MissMat += Miss
            correct1 += result1
