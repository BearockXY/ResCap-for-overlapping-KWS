...
def show_reconstruction(model, test_loader, n_images, args):
    ...
    for x, _, _ in test_loader:
        ...
        white = np.random.random_sample([98,60])
       ...
       # plt.imshow(plt.imread(args.save_dir + "/real_and_recon.png", ))
        x_recon = x_recon.cpu()
        fb_feature = x_recon[0][0].detach().numpy()*320
        # print(x_recon[0][0].detach().numpy())
        fb_feature = fb_feature + white*50

        recon_feature = np.fliplr(0.71*fb_feature.T)
        librosa.display.specshow(recon_feature,cmap='coolwarm',x_axis='time',y_axis='mel',fmin=0.0,fmax=4000.0)
        plt.title('\'Visual\' from Reconstruction')
        # plt.title('Overlapped \'Six\' and \'Visual\' from Reconstruction')
