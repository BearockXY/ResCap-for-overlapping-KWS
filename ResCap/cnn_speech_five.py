...
from fbanks_load_5 import load_mnist,read
...
def Find_correct(inputdata,target1,target2):
    ...
    result1, result2,Miss = judge_correct(y,z,index_pred1.float(),index_pred2.float())
...
def test(args, model,device,train_loss,test_loader,epoch,ti):
    ...
    with torch.no_grad():
        for data,target1,target2 in test_loader:
            ...
            result1,result2,Miss = Find_correct(output,target1,target2)
            MissMat += Miss
            correct1 += result1
            correct2 += result2
    ...
    print(MissMat)
    # loss /=
    print('epoch:{:.0f},train_loss:{:.5f},Test set: Average loss: {:.5f}, Accuracy 1: {}/{} ({:.2f}%),Accuracy 2: {}/{} ({:.2f}%),Time is {:.4f}\n'.format(
        epoch, train_loss,test_loss, correct1, len(test_loader.dataset),
        100. * correct1 / len(test_loader.dataset),correct2, len(test_loader.dataset), 100. * correct2 / len(test_loader.dataset),
        time()-ti))
