...
from fbanks_load_5_mix4 import load_mnist,read


def order4(input1,input2,input3,input4):
    # print("=======================================")
    tag1,tag2,tag3 = order3(input1,input2,input3)
...
def judge_correct(label1,label2,label3,label4,pred1,pred2,pred3,pred4):
    label1 = label1.long()
    label2 = label2.long()
    label3 = label3.long()
    label4 = label4.long()
    ...
    for i in range(len(label1)):
        ...
        tag1,tag2,tag3,tag4 = order4(pred1[i],pred2[i],pred3[i],pred4[i])
        print(tag1,tag2,tag3,tag4)
        ans1,ans2,ans3,ans4 = order4(label1[i],label2[i],label3[i],label4[i])
        Miss[tag1][ans1] +=1
        Miss[tag2][ans2] +=1
        Miss[tag3][ans3] +=1
        Miss[tag4][ans4] +=1
...
def find_index(input_tensor):
    ...
    for i in range(len(index3)):
        maxvalue2[i] = input_tensor[i][index3[i]]
...
class SpeechResModel(nn.Module):
    ...
    def forward(self, x,y_p=None):
        ...
        if y_p is None:  # during testing, no label given. create one-hot coding using `length`
            index1, index2,index3,index4 = find_index(length)
            ...
            y3 = torch.zeros(length.size()).scatter_(1, index3.view(-1, 1).cpu().data, 1.)
            y4 = torch.zeros(length.size()).scatter_(1, index4.view(-1, 1).cpu().data, 1.)
            index =torch.add(y1,y2)
            index =torch.add(index,y3)
            index =torch.add(index,y4)
...
def test(model, test_loader, args):
    ...
    correct2 = 0
    correct3 = 0
    MissMat = np.zeros([5,5])
    for x, y, z, w,v in test_loader:
        ...
        w_hot = torch.zeros(w.size(0), 5).scatter_(1, w.view(-1, 1), 1.)
        v_hot = torch.zeros(v.size(0), 5).scatter_(1, w.view(-1, 1), 1.)
        true_label = torch.add(y_hot,z_hot)
        true_label = torch.add(w_hot,true_label)
        true_label = torch.add(v_hot,true_label)
        y, z, w,v = Variable(y.float().cuda()), Variable(z.float().cuda()), Variable(w.float().cuda()), Variable(v.float().cuda())
        ...
        index_pred1,index_pred2,index_pred3,index_pred4 = find_index(y_pred)
        ...
        result,Miss = judge_correct(y,z,w,v,index_pred1.float(),index_pred2.float(),index_pred3.float(),index_pred4.float())
        MissMat  +=Miss
        correct1 +=result
...
if __name__ == "__main__":
    ...
    # define model
    model = SpeechResModel(input_size=[1, 98, 60], classes=5, routings=3)
    model.cuda()
    print(model)
    ...
    if not args.testing:
        ...
        val_loss, val_acc1 = test(model, test_loader, args)
        print('test acc1 = %.5f, test loss = %.5f' % (val_acc1, val_loss))
