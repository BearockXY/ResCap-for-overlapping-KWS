...
from fbanks_load_10_sep import load_mnist,read
...
def judge_correct(label1,pred1,label2,pred2):
    # correct1 = 0
    # correct2 = 0
    ...
    return correct1,correct2,Miss

def Find_correct(inputdata1,target1,inputdata2,target2):
    correct1= 0
    correct2 =0
    MissMat = np.zeros([12,12])

    index_pred1= find_index(inputdata1)
    index_pred2= find_index(inputdata2)
    # y,z = Variable(target1.float().cuda()), Variable(target2.float().cuda())
    y1 = Variable(target1.float())
    y2 = Variable(target2.float())

    result1,result2,Miss = judge_correct(y1,index_pred1.float(),y2,index_pred2.float())
    ...
    return correct1,correct2, MissMat

def CELoss(inputdata1,inputdata2, target1,target2):
    criterion = nn.CrossEntropyLoss()
    #
    # loss1_1 = criterion(inputdata1,target1)
    # loss1_2 = criterion(inputdata1,target2)
    # loss2_1 = criterion(inputdata2,target1)
    # loss2_2 = criterion(inputdata2,target2)
    # loss1 = torch.min(loss1_1,loss1_2)
...
def train(args, model, device, train_loader,test_loader, optimizer, epoch,ti):
    ...
    model.train()
    train_loss = 0
    for batch_idx, (data1,data2,target1,target2) in enumerate(train_loader):

        data1 = data1.view_as(torch.Tensor(args.batch_size,1,98,60)).float()
        data1, target1 = data1.to(device), target1.to(device)

        data2 = data2.view_as(torch.Tensor(args.batch_size,1,98,60)).float()
        ...
        loss = CELoss(output1,output2, target1,target1)
...
def test(args, model,device,train_loss,test_loader,epoch,ti):
    ...
    MissMat = np.zeros([12,12])
    with torch.no_grad():
        for data1,data2,target1,target2 in test_loader:
            ...
            output1 = model(data1)
            output2 = model(data2)
            test_loss += CELoss(output1,output2,target1,target2)*data1.size(0) # sum up batch loss
            # pred = output.max(1, keepdim=True)[1] # get the index of the max log-probability
            # correct += pred.eq(target.view_as(pred)).sum().item()
            result1,result2,Miss = Find_correct(output1,target1,output2,target2)
            MissMat += Miss
            correct1 += result1
            correct2 += result2
    ...
    for i in range(len(MissMat)):
        for j in range(len(MissMat[0])):
            MissMat[i][j] = int(MissMat[i][j])
    np.set_printoptions(suppress=True)
    print(MissMat)

    print('epoch:{:.0f},train_loss:{:.5f},Test set: Average loss: {:.5f}, Accuracy1: {}/{} ({:.2f}%), Accuracy2: {}/{} ({:.2f}%),Time is {:.4f}\n'.format(
        epoch, train_loss,test_loss,correct1, len(test_loader.dataset),
        100. * correct1 / len(test_loader.dataset), correct2, len(test_loader.dataset),
