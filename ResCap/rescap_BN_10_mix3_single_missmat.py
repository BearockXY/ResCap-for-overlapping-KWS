...
def judge_correct(label1,label2,label3,pred1,pred2,pred3):
    label1 = label1.long()
    label2 = label2.long()
    label3 = label3.long()
    pred1 = pred1.long()
    ...
    for i in range(len(label1)):
        ...
        if tag1 == ans1:
            ...
            if tag2 == ans2:
                ...
                if tag3 == ans3:
                    correct1 +=1
    return correct1,correct2,correct3,Miss
    # correct = 0
    # for i in range(len(label1)):
    #     if label1[i] == pred1[i]:
    #         if label2[i] == pred2[i]:
...
class SpeechResModel(nn.Module):
    ...
    def forward(self, x,y_p=None):
        ...
        if y_p is None:  # during testing, no label given. create one-hot coding using `length`
            index1, index2,index3 = find_index(length)
...
def test(model, test_loader, args):
    ...
    correct2 = 0
    correct3 = 0
    MissMat = np.zeros([10,10])
    for x, y, z, w in test_loader:
        ...
        w_hot = torch.zeros(w.size(0), 10).scatter_(1, w.view(-1, 1), 1.)
        true_label = torch.add(y_hot,z_hot)
        true_label = torch.add(w_hot,true_label)
        y, z, w = Variable(y.float().cuda()), Variable(z.float().cuda()), Variable(w.float().cuda())
        ...
        index_pred1,index_pred2,index_pred3 = find_index(y_pred)
        ...
        result1, result2, result3,Miss = judge_correct(y,z,w,index_pred1.float(),index_pred2.float(),index_pred3.float())
        MissMat  +=Miss
        correct1 +=result1
        correct2 +=result2
        correct3 +=result3
...
def train(model, train_loader, test_loader, args):
    ...
    for epoch in range(args.epochs):
        ...
        logwriter.writerow(dict(epoch=epoch, loss=training_loss / len(train_loader.dataset),
                                val_loss=val_loss, val_acc=val_acc1))
        print("==> Epoch %02d: loss=%.5f, val_loss=%.5f, val_acc1=%.4f, time=%ds"
              % (epoch, training_loss / len(train_loader.dataset),
                 val_loss, val_acc1, time() - ti))
        if val_acc1 > best_val_acc1:  # update best validation acc and save model
            best_val_acc1 = val_acc1
            torch.save(model.state_dict(), args.save_dir + '/epoch%d.pkl' % epoch)
            print("best val_acc2 increased to %.4f" % best_val_acc1)
    logfile.close()
...
if __name__ == "__main__":
    ...
    if not args.testing:
        ...
        val_loss, val_acc1,val_acc2,val_acc3 = test(model, test_loader, args)
        print('test acc1 = %.5f,test acc2 = %.5f,test acc3 = %.5f, test loss = %.5f' % (val_acc1,val_acc2,val_acc3, val_loss))
